...
} from '@cloudpaste/shared/components';
import { clipboardApi, formatDate, copyToClipboard, truncateText, isPreviewableTextFile, isPreviewableMediaFile, getMediaType, downloadFile, beautifyContent, looksLikeCode } from '@cloudpaste/shared/lib';
import { useClipboardStore } from '@cloudpaste/shared/store';
import { useWebSocketContext } from '@cloudpaste/shared/contexts';
...
export default function DashboardPage() {
  ...
  const { syncClipboard } = useWebSocketContext();
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.content_type === 'image' ? (
                      <ClipboardImageItem
                        ...
                        onClick={() => setPreviewImage({ url: item.content, name: item.file_name })}
                      />
                  ...
                  <div className="flex gap-1 sm:gap-2 flex-shrink-0">
                    ...
                    {item.content_type !== 'text' && (
                      <Button
                        ...
                        onClick={async () => {
                          try {
                            await downloadFile(item.content, item.file_name);
                            toast.success('下载成功');
                          } catch (error) {
                            toast.error('下载失败');
                          }
