/**
 * 通用文件缓存 Hook（Tauri Desktop）
 * 自动下载并缓存远程文件，加载过的文件从本地读取
 ...
 * 通用文件缓存 Hook
 */
export function useFileCache(fileUrl: string | null) {
      ...
      try {
        ...
        try {
          ...
          if (cachedPath.startsWith('/') || cachedPath.includes(':\\')) {
            ...
            setCachedUrl(assetUrl);
          } else {
            // 如果下载失败，返回的是原始 URL
            setCachedUrl(cachedPath);
          }
        } catch (err) {
          console.error('文件缓存失败，使用原始 URL:', err);
          // 缓存失败时使用原始 URL
          setCachedUrl(apiUrl);
        }
      } catch (err) {
